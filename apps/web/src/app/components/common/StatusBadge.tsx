...
export const StatusBadge: React.FC<StatusBadgeProps> = ({ status }) => {
  const getTheme = (s: string) => {
    ...
    if (low === 'problem') return { bg: 'bg-yellow-50 text-yellow-700 border-yellow-200', dot: 'bg-yellow-500' };
    if (['declined', 'denied', 'cancelled'].includes(low)) return { bg: 'bg-red-50 text-red-700 border-red-200', dot: 'bg-red-500' };
  ...
  return (
    <span className={`inline-flex items-center px-4 py-1.5 rounded-full text-[10px] font-black border ${theme.bg} uppercase tracking-[0.1em] flex-shrink-0 whitespace-nowrap`}>
      ...
      {(low === 'expired' && !window.location.pathname.includes('/admin')) ? 'Not Accepted' : status}
    </span>
  );
};
