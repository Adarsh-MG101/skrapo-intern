...
import { AlertCircle, CheckCircle2, Eye, EyeOff, UserPlus, X } from 'lucide-react';
...
function CreateChampContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setSuccess('Scrap Champion created successfully! Redirecting...');
        setTimeout(() => {
      ...
      setError('Connection error or invalid data. Please check your internet and try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-4 md:p-8 lg:p-10 bg-gray-50/20 min-h-screen">
      <div className="max-w-2xl mx-auto">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-black text-gray-900 tracking-tight flex items-center gap-2">
              <UserPlus className="text-brand-500" /> Create Champion
            </h1>
            <p className="text-gray-500 mt-2 font-medium">Add a new partner to the network</p>
          </div>
          <button 
            onClick={() => router.back()}
            className="p-3 bg-white rounded-2xl border border-gray-100 text-gray-500 hover:text-red-500 transition-all shadow-sm flex items-center gap-2 px-6 font-bold text-sm"
          >
            <X size={18} /> Cancel
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-[2.5rem] p-8 md:p-10 shadow-xl shadow-gray-200/30 border border-gray-100 space-y-8">
          ...
          <Button 
            ...
            size="lg" 
            isLoading={loading}
            className="py-5 rounded-[1.5rem] shadow-xl shadow-brand-500/20 active:scale-95 transition-transform"
          >
...
export default function CreateChampPage() {
  return (
